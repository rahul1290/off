...
class Kra_ctrl_old extends CI_Controller {
	...
	function index($session=null,$ecode=null){
	    if($_SERVER['REQUEST_METHOD'] === 'POST'){
			...
			if($this->input->post('weightage5') != ''){
				$this->form_validation->set_rules('key_result_area5','Key Result Area5','trim|required');
			} else{
			...
			if($this->input->post('weightage6') != ''){
				$this->form_validation->set_rules('key_performance_indicator6','key performance Indicator6','trim|required');
			} else{
				$this->form_validation->set_rules('key_performance_indicator6','key performance Indicator6','trim');
			}
	        ...
	        if ($this->form_validation->run() == FALSE){
				...
				redirect('es/KRA_old/'.base64_encode($data['session'][0]['name']).'/'.base64_encode($ecode));
