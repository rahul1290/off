					...
					<div class="col-2">
              		...
              		<div class="col-md-10 col-xs-12">
              			<table class="table table-bordered table-striped">
              				...
              				<form id="f1" method="POST" action="<?php echo base_url('es/KRA_old/').$this->uri->segment(3).'/'.base64_encode($user_detail[0]['ecode']); ?>" onsubmit="return form_submit()">
              			</table>
              		</div>
	...
	function form_submit(){
		...
		$('.target').each(function(){
			...
			if($('#target'+id).hasClass('visible')){
				...
				else if(parseFloat($('#target'+id).val()) == '0'){
					counter = counter + 1;
					$('#target'+id+'_errors').html('Target not to be Zero').show();
				}
